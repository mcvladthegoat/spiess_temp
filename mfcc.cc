...
public:
    ...
    std::string processFrame(int16_t* samples, size_t N) {
        ...
        preEmphHam(); // 1. Process frames with Hamming windows
        computePowerSpec(); // 2. Compute the power of spectrum
        applyLMFB(); // 3. 
        applyDct(); // 4. Discrete cosine transform
    ...
    int process (std::ifstream &wavFp, std::ofstream &mfcFp) {
        ...
        // mfcFp << "\nwinLengthSamples" + winLengthSamples;
